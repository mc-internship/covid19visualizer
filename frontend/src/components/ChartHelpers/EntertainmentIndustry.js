...
function EntertainmentChart(props) {
  const dates = [];
  ...
  props.timeseries.forEach((data, index) => {
      ...
      dates.push(date);
      entertainmentindex.push(data.entertainment);
  ...
  const dataset = {
    ...
    datasets: [
      {
        borderWidth: 2,
        data: entertainmentindex,
        borderCapStyle: 'round',
        pointBackgroundColor: '#33ffff',
        label: 'Entertainment Industry Index',
        borderColor: '#33ffff',
        pointHoverRadius: 4,
      }
    ],
